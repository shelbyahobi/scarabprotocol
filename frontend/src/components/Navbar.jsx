        ...
        <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? 'py-4' : 'py-6'} px-4 md:px-8`}>
            ...
            <div className={`mx-auto max-w-7xl flex justify-between items-center transition-all duration-300 ${scrolled ? 'bg-[#050B08]/80 backdrop-blur-xl border border-white/10 shadow-2xl py-3 px-6 rounded-2xl' : 'bg-transparent py-2 border border-transparent'}`}>
                ...
                <div className="hidden md:flex items-center gap-1 bg-white/5 border border-white/5 rounded-full px-2 py-1.5 backdrop-blur-md">
                    <Link
                        to="/products"
                        className={`flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold transition-all ${location.pathname === '/products' ? 'bg-white/10 text-white' : 'text-gray-300 hover:text-white hover:bg-white/10'}`}
                    >
                    ...
                    <Link
                        to="/marketplace"
                        className={`flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold transition-all ${location.pathname === '/marketplace' ? 'bg-white/10 text-white' : 'text-gray-300 hover:text-white hover:bg-white/10'}`}
                    >
            ...
            {mobileMenuOpen && (
                <div className="md:hidden absolute top-full left-4 right-4 mt-2 bg-[#050B08]/95 border border-white/10 p-6 rounded-2xl flex flex-col gap-6 backdrop-blur-2xl shadow-2xl">
                    <div className="flex flex-col gap-2">
                        <Link to="/products" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 text-lg font-bold text-gray-200 hover:text-white p-3 rounded-xl hover:bg-white/5 transition-colors">
                            <Rocket size={20} className="text-beetle-electric" /> Products
                        </Link>
                        ...
                        <Link to="/marketplace" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 text-lg font-bold text-gray-200 hover:text-white p-3 rounded-xl hover:bg-white/5 transition-colors">
                        ...
                        <Link to="/dao" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 text-lg font-bold text-gray-200 hover:text-white p-3 rounded-xl hover:bg-white/5 transition-colors">
                            <Users size={20} className="text-purple-400" /> DAO
                        </Link>
                    </div>
